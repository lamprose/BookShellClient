...
//                || Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
        ...
        perArr.forEach { permissions[it] = Permission(it, false, false) }
    ...
    fun onRequestPermissionsResult(requestCode: Int, pers: Array<out String>, grantResults: IntArray) {
        if (requestCode == PERMISSIONS_REQUEST_CODE && grantResults.isNotEmpty()) {
            ...
            for (index in grantResults.indices) {
                log("onRequestPermissionsResult---" + pers[index] + "---" + grantResults[index])
                val permission = permissions[pers[index]]

                permission?.let {
                    permission.granted = grantResults[index] == PackageManager.PERMISSION_GRANTED
        ...
        activity?.let {
            unPermissions.forEach {
                if (ActivityCompat.shouldShowRequestPermissionRationale(activity!!, it)) {
                    ...
                    permissions[it]?.shouldShowRequestPermission = true
                    onRequestPermissionsCallback(permissions[it]!!)
                }
            }
        } ?: unPermissions.forEach {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M &&
